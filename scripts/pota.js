 ...
 function renderLeaders(data){
  const render=(id,rows)=>{
    ...
    el.innerHTML=rows.map((r,i)=>{
      const call=String(r.callsign||'').toUpperCase();
      const cls=call==='K5KAZ'?'pota-leader-call is-you':'pota-leader-call';
      return '<div class=\"pota-leader-row\"><span class=\"pota-leader-rank\">'+(i+1)+'.</span><span class=\"'+cls+'\">'+call+'</span><span class=\"pota-leader-value\">'+Number(r.count||0).toLocaleString()+'</span></div>';
    }).join('');
  };
